...
(function() {
  ...
  Service = (function() {
      ...
      this.heartbeatInterval = setInterval((function(_this) {
        return function() {
          ...
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            ...
            _this.send(host, "heartbeat", 200, {
              service: _this.name
            });
            _results.push(connection.heartbeat++);
          }
          return _results;
        };
      })(this), 1000);
      this.heartbeatResetInterval = setInterval((function(_this) {
        return function() {
          ...
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            ...
            if (connection.heartbeat > 2) {
              _this.hosts.splice(_this.hosts.indexOf(connection), 1);
              _results.push(console.dir("disconnected"));
            } else {
    ...
    Service.prototype.receive = function(publicInfo, data) {
      var connection, _i, _len, _ref, _results;
      _ref = this.hosts;
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        connection = _ref[_i];
        if (connection["private"].port === publicInfo.port && connection["private"].address === publicInfo.address) {
  ...
  UDPHoleService = (function() {
    function UDPHoleService(options) {
      ...
      this.handleServiceHeartbeat = __bind(this.handleServiceHeartbeat, this);
      ...
      return this.listener.socket.on("message", (function(_this) {
        return function(encodedData, publicInfo) {
          ...
          if (data.request === "connect") {
            _this.handleServiceConnection(data, publicInfo);
          }
          if (data.request === "heartbeat") {
            return _this.handleServiceHeartbeat(data, publicInfo);
          }
        };
      })(this));
    };
    ...
    UDPHoleService.prototype.handleServiceHeartbeat = function(data, publicInfo) {
      var service;
      service = this.services[data.service];
      if (service != null) {
        return service.receive(publicInfo, data);
      }
    };
